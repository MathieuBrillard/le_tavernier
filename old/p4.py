...
def board_update(old_board, gameboard, color, joueur, ligne, colonne):
    ...
    board = Image.new(
        'RGBA', (grille_size[0], grille_size[1]), (250, 250, 250, 0))
...
async def P4(ctx, j2, bot):
    ...
    gameboard = numpy.array(numpy.zeros(shape=[6, 7], dtype=int))
    # Choix de la couleur :
    ...
    while game:
        ...
        if ligne == -1:
            ...
            old_board = board
            board, gameboard = board_update(
                old_board, gameboard, jeton, joueur, ligne, colonne)
            ...
            if is_winner(gameboard, joueur) == False:
                ...
                if joueur == "j1":
                    embed.set_footer(
                        text=f'C\'est au tour de {j2}.\nRéagissez pour jouer.')
                else:
                    embed.set_footer(
                        text=f'C\'est au tour de {j1}.\nRéagissez pour jouer.')
                embed.set_image(url="attachment://board.png")
                msg = await ctx.send(embed=embed, file=file)
                await msg.add_reaction('1️⃣')
                await msg.add_reaction('2️⃣')
