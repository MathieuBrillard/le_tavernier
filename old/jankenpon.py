...
async def Jankenpon(ctx, j2, bot):
    ...
    embed = discord.Embed(
        title=f'Pierre Feuille Ciseau _VS_  {j2}', description=desc, color=discord.Color.blue())
    reply_j1 = await j1.send(embed=embed)
    embed = discord.Embed(
        title=f'Pierre Feuille Ciseau _VS_  {j1}', description=desc, color=discord.Color.blue())
    ...
    if winner == 0:
        ...
        embed = discord.Embed(
            title=f'{j1} a joué {c_j1} _VS_  {j2} a joué {c_j2}', description=desc, color=discord.Color.blue())
